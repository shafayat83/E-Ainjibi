...
export default function App() {
  ...
  const handleSend = async (text: string = input, isEdit: boolean = false, editIdx: number | null = null) => {
    ...
    try {
      // Robust API key access for both AI Studio and standard Vite/Vercel deployments
      const apiKey = (import.meta.env?.VITE_GEMINI_API_KEY as string) || 
                     (typeof process !== 'undefined' ? process.env?.GEMINI_API_KEY : '') || 
                     '';
      
      if (!apiKey) {
        throw new Error("API Key not found. Please set VITE_GEMINI_API_KEY in your Vercel/Environment settings.");
      }
      ...
      const botText = response.text || "দুঃখিত, আমি উত্তর দিতে পারছি না।";
